...
{
    public class NewItemHomeViewComponent : ViewComponent
    {
        private readonly IRepository<NewsItem> _newsItemRepository;
        private readonly IMediaService _mediaService;

        public NewItemHomeViewComponent(IRepository<NewsItem> newItemRepository,
        ...
        public IViewComponentResult Invoke()
        {
            ...
            var query = _newsItemRepository.Query().Include(x => x.ThumbnailImage)
                .Where(x => !x.IsDeleted && x.IsPublished).OrderByDescending(m => m.CreatedOn).Take(5);
